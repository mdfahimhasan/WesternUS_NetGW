...
predictor_WestUS = create_dataframe_csv(input_data_dir='../Data_main/Compiled_data',
                                        ...
                                        skip_dataframe_creation=skip_df_creation, encode_cols=None)

# Creating train, test, and validation dataset csv
data_frac, train_frac, val_frac, test_frac = 0.7, 0.7, 0.15, 0.15  # #
drop_predictor = []  # dropped attributes are are one-hot encoded and not necessary
train_val_test_exists = False  # # Set to False to create train, test, and validation dataset csv
...
rank = 0  # #
world_size = 1  # #
batch_size = 64  # batch_size optimization helps with CUDA memory error_mae
verbose = True  # #
print_epoch = 10  # #
skip_train = False  # #
setup_ddp = True  # #
