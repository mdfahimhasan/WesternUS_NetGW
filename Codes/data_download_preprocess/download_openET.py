...
def get_openet_gee_dict(data_name):
    ...
    gee_data_dict = {
        ...
        'Natural_OpenET_IrrMapper': 'OpenET/ENSEMBLE/CONUS/GRIDMET/MONTHLY/v2_0',
        'Natural_OpenET_LANID': 'OpenET/ENSEMBLE/CONUS/GRIDMET/MONTHLY/v2_0',
    ...
    gee_band_dict = {
        ...
        'Natural_OpenET_IrrMapper': 'et_ensemble_mad',  # unit in mm, monthly total
        'Natural_OpenET_LANID': 'et_ensemble_mad',  # unit in mm, monthly total
        'USDA_CDL': 'cropland',
    ...
    gee_scale_dict = {
        ...
        'Natural_OpenET_IrrMapper': 1,
        'Natural_OpenET_LANID': 1,
        'USDA_CDL': 1,
        'IrrMapper': 1,
        'LANID': 1,
        'AIM-HPA': 1,
    ...
    aggregation_dict = {
        ...
        'Natural_OpenET_IrrMapper': ee.Reducer.sum(),
        'Natural_OpenET_LANID': ee.Reducer.sum(),
    ...
    month_start_date_dict = {
        ...
        'Natural_OpenET_IrrMapper': datetime(2016, 1, 1),
        'Natural_OpenET_LANID': datetime(2016, 1, 1),
    ...
    month_end_date_dict = {
        ...
        'Natural_OpenET_IrrMapper': datetime(2021, 1, 1),
        'Natural_OpenET_LANID': datetime(2021, 1, 1),
...
.
    ...
    # Extracting dataset information required for downloading from GEE
    lanid_asset, _, _, _, _, _, _, _ = get_openet_gee_dict(data_name)
    ...
    AimHpa_asset, _, _, _, _, _, _, _ = get_openet_gee_dict('AIM-HPA')
...
def download_fallow_ET_from_OpenET_IrrMapper_monthly(data_name, download_dir, year_list, month_range, grid_shape,
                                                     num_pts_each_grid=5000, use_cpu_while_multidownloading=15):
    ee.Initialize(opt_url='https://earthengine-highvolume.googleapis.com')
    download_dir = os.path.join(download_dir, data_name)
    makedirs([download_dir])
    ...
    for year in year_list:  # first loop for years
        # # # Extracting fallow land cover dataset from USDA CDL data and filtering out any irrigated using IrrMapper

        # # IrrMapper data for the year
        irrmap_imcol = ee.ImageCollection(irr_data)
        irrmapper_img = irrmap_imcol.filter(ee.Filter.calendarRange(year, year, 'year')).select(irr_band) \
            .reduce(irr_reducer)
        # In IrrMapper dataset non-irrigated fields are assigned as 1.
        # We will use that to mask out irrigated pixels that are assigned as fallow in cdl
        non_irrig_mask = irrmapper_img.eq(1)

        # # USDA CDL data for the year
        cdl_img = ee.ImageCollection(cdl_data).filter(ee.Filter.calendarRange(year, year, 'year')) \
            .first().select(cdl_band)

        # cdl fallow land cover and masking out any irrigated pixel
        fallow = cdl_img.eq(61)
        fallow = fallow.updateMask(fallow)
        fallow = fallow.updateMask(non_irrig_mask)
        ...
        for month in month_list:  # second loop for months
            ...
            if (start_date_dt >= et_month_start_range) & (end_date_dt <= et_month_end_range):
                ...
                for i in range(len(grid_no)):  # third loop for grids
                    ...
                    # Sampling random points over fallow land var
                    fallow_pts = fallow.stratifiedSample(numPoints=num_pts_each_grid, region=gee_extent, seed=0,
                                                         classValues=[1], classPoints=[num_pts_each_grid],
                                                         dropNulls=True, geometries=True)

                    # extracting openET values over fallow points
                    pts_ET = openET_img.reduceRegions(collection=fallow_pts, reducer=ee.Reducer.mean(),
                                                      crs='EPSG:4269', scale=30)

                    # Getting Data URl for each grid from GEE
                    # The GEE connection gets disconnected sometimes, therefore, we adding the try-except block to retry
                    # failed connections
                    try:
                        data_url = pts_ET.getDownloadURL(filename=data_name)
                    except:
                        data_url = pts_ET.getDownloadURL(filename=data_name)

                    local_file_path = os.path.join(download_dir,
                                                   f'{data_name}_{str(year)}_{str(month)}_{str(grid_sr)}.csv')
...
def download_fallow_ET_from_OpenET_LANID_monthly(data_name, download_dir, year_list, month_range, grid_shape,
                                                 num_pts_each_grid=5000, use_cpu_while_multidownloading=15):
    ee.Initialize(opt_url='https://earthengine-highvolume.googleapis.com')
    download_dir = os.path.join(download_dir, data_name)
    makedirs([download_dir])

    # Extracting OpenET, CDL, LANID & AIM-HPA dataset information required for downloading from GEE
    # Extracting LANID dataset information (saved as an asset) from GEE
    lanid_asset, _, _, _, _, _, _, _ = get_openet_gee_dict('LANID')
    lanid_data_band_dict = {2000: 'lanid_2000', 2001: 'lanid_2001', 2002: 'lanid_2002', 2003: 'lanid_2003',
                            2004: 'lanid_2004', 2005: 'lanid_2005', 2006: 'lanid_2006', 2007: 'lanid_2007',
    ...
    for year in year_list:  # first loop for years
        ...
        # In the joined irrigation dataset (LANID + AIM-HPA) dataset non-irrigated fields are assigned as 0.
        # We will use that to mask out irrigated pixels that are assigned as fallow in cdl
        non_irrig_mask = irr_total.eq(0)
...
def download_openET_data(data_list, download_dir, year_list, month_range,
    ...
    if not skip_download:
        for data_name in data_list:
            if data_name == 'Irrig_crop_OpenET_IrrMapper':
                ...
                download_Rainfed_CropET_from_OpenET_IrrMapper_monthly(data_name=data_name, download_dir=download_dir,
                                                                      ...
                                                                      use_cpu_while_multidownloading=use_cpu_while_multidownloading)
            elif data_name == 'Natural_OpenET_IrrMapper':
                download_fallow_ET_from_OpenET_IrrMapper_monthly(data_name=data_name, download_dir=download_dir,
                                                                 year_list=year_list, month_range=month_range,
                                                                 grid_shape=grid_shape_for30m_irrmapper,
                                                                 num_pts_each_grid=5000,
                                                                 use_cpu_while_multidownloading=use_cpu_while_multidownloading)

            elif data_name == 'Natural_OpenET_LANID':
                download_fallow_ET_from_OpenET_LANID_monthly(data_name=data_name, download_dir=download_dir,
                                                             year_list=year_list, month_range=month_range,
